...
| IndexedDB full backup/restore/transfer | custom `exportFullBackup` / `importFullBackup`, `HordeDB` payloads | 17.4 already adds chat attachment media to the full backup | Preserve 17.4's chat-media coverage, carry forward the custom roleplay-OS/shared application records where still global, then replace the flat export with registered opaque-domain coordination before Experimental is attached. | In progress |
