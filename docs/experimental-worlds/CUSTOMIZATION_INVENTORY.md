...
| People, Locations, graduation, relationship metadata/meters, visuals, outfits | Authoritative custom World implementation | Not substituted with pristine helpers | Experimental Worlds | Existing custom World schemas/media; visual briefs are globally persisted settings but retain Experimental semantics | Visual/media, structured documents, outfits, portrait request composition, and portable media assets relocated mechanically through a narrow settings/dirty-state adapter; real Visuals tab browser-proved without a provider call | Pass-1 relocation in progress; People/Locations/graduation UI remains in-place |
