/*
 * Milestone-A browser regression for the real Melbourne reroll finding. The
 * provider is deliberately mocked, but world creation, send, reroll, receipt,
 * persistence, and reload run through the preserved dual-17.0 UI.
 */
const { chromium } = require(process.env.HORDE_PLAYWRIGHT_MODULE);

const port = process.env.HORDE_ACCEPTANCE_PORT || '43132';
const mockPort = process.env.HORDE_MOCK_PORT || '43205';
const worldName = `Temporal Reroll 17.0 Disposable ${Date.now()}`;
...
(async () => {
    ...
    try {
        ...
        must((await page.locator('.msg-dm').last().textContent()).includes('Time 6:32 PM'),
            'the dual-17.0 browser did not render the deliberately malformed same-Day header');
