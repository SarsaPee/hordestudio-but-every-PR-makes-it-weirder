/* Synthetic-only staged 17.4 cross-mode deletion/readback acceptance. */
const { chromium } = require(process.env.HORDE_PLAYWRIGHT_MODULE);

const port = process.env.HORDE_ACCEPTANCE_PORT || '43136';
const suffix = Date.now();
const stockWorld = `Stock Delete 17.4 Disposable ${suffix}`;
const experimentalWorld = `Experimental Counterpart 17.4 Disposable ${suffix}`;
const stockCounterpart = `Stock Counterpart 17.4 Disposable ${suffix}`;
const experimentalCounterpart = `Experimental Delete 17.4 Disposable ${suffix}`;
...
async function deleteWorld(page, name) {
    ...
    const dialogHandled = new Promise(resolve => page.once('dialog', async dialog => {
        dialogType = dialog.type(); await dialog.accept(); resolve();
    }));
