...
test('the repair runs on load, import, save, export and delete', () => {
    ...
    assert(/normalizeAuthoredWorld\(ExperimentalWorldsState\.editingWorld\);[\s\S]{0,700}?const data = JSON\.stringify\(exportedWorld/.test(app),
    ...
    assert(/removeWorldLocationRecord\(ExperimentalWorldsState\.editingWorld, loc\.id\);[\s\S]{0,300}?normalizeAuthoredWorld/.test(app),
