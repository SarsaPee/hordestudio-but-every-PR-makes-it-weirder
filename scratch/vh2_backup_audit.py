...
class Backup(unittest.TestCase):
 ...
 def test_workspace_replace_is_atomic_and_removes_post_backup_lives(self):
  second=self.new_source('Second life')
  before=vh2_backup.export_workspace(self.source)
  stale=self.new_source('Created after backup')
  result=vh2_backup.replace_workspace(self.source,before['archives'])
  self.assertEqual({row['worldId'] for row in result['worlds']},set(before['archives']))
  self.assertEqual({row['worldId'] for row in self.source.status()['worlds']},set(before['archives']))
  self.assertNotIn(stale,{row['worldId'] for row in self.source.status()['worlds']})
  for world_id in (self.w,second):self.assertEqual(self.source.projection(world_id)['state'],self.source.replay(world_id))
  intact=vh2_backup.export_workspace(self.source)
  invalid={**intact['archives'],'not-the-archive-world':next(iter(intact['archives'].values()))}
  with self.assertRaises(ValueError):vh2_backup.replace_workspace(self.source,invalid)
  self.assertEqual(set(vh2_backup.export_workspace(self.source)['archives']),set(intact['archives']))
