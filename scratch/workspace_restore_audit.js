...
assert.match(app, /activeVideoWorldSessionId: workspaceString\(state\.activeVideoWorldId/);
assert.match(app, /activeCompanionSessionId: workspaceString\(state\.activeCompanionId/);
assert.match(app, /companionStudioTabsById: workspaceTabMap\(state\.companionStudioTabsById\)/);
assert.match(app, /companionSocialTabsById: workspaceTabMap\(state\.companionSocialTabsById\)/);
assert.match(app, /settingsOpen: state\.settingsOpen === true/);
assert.match(app, /settingsSection: SETTINGS_SECTION_LABELS/);
assert.match(app, /function openWorldStudio\(worldId = null\)/);
assert.match(app, /if \(workspaceRestoring\) return;/);
...
assert.match(html, /app\.js\?v=17\.4\.0-workspace-restore-4/);
