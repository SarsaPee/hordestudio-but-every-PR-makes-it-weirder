...
def workspace_archives(archives):
 """Decode a bounded browser backup payload before opening a service write."""
 ...
 try:return [base64.b64decode(a,validate=True) for a in archives]
 except (ValueError,TypeError):raise ValueError('Invalid workspace archive encoding.') from None

def workspace_archive_map(archives):
 """Decode a keyed full-service backup without losing world identity."""
 if not isinstance(archives,dict) or len(archives)>100:raise ValueError('Invalid workspace timeline archives.')
 if any(not isinstance(world_id,str) or not 1<=len(world_id)<=100 or not isinstance(data,str)
        for world_id,data in archives.items()):raise ValueError('Invalid workspace timeline archives.')
 if sum(map(len,archives.values()))>256*1024*1024:raise ValueError('Workspace service archives exceed 256 MB.')
 try:decoded={world_id:base64.b64decode(data,validate=True) for world_id,data in archives.items()}
 except (ValueError,TypeError):raise ValueError('Invalid workspace archive encoding.') from None
 if any(archive_world_id(data)!=world_id for world_id,data in decoded.items()):
  raise ValueError('Workspace archive identity does not match its world id.')
 return decoded

def export_workspace(service):
 """Export every VH2 life as the opaque, keyed payload for host backup."""
 with service.connect() as db:
  world_ids=[row[0] for row in db.execute('SELECT id FROM worlds ORDER BY id').fetchall()]
 archives={};total=0
 for world_id in world_ids:
  data=export(service,world_id);total+=len(data)
  if total>MAX_COMPRESSED:raise ValueError('Workspace service archives exceed 256 MB.')
  archives[world_id]=base64.b64encode(data).decode('ascii')
 return {'archives':archives}

def restore_workspace(service,archives):
 """Restore an archive set only when every world is new to this service."""
 from contextlib import contextmanager
 decoded=workspace_archives(archives)
 with service.connect() as db:
...
def archive_world_id(data):
 """Read only the identity needed to replace a known service world.

 Full integrity and replay validation remains in ``restore`` inside the same
 SQLite transaction, so an invalid later archive rolls every earlier delete
 back as well.
 """
 try:
  with gzip.GzipFile(fileobj=io.BytesIO(data)) as stream:raw=stream.read(MAX_RAW+129)
  if len(raw)>MAX_RAW+128:raise ValueError()
  world_id=json.loads(raw)['payload']['worldId']
  if not isinstance(world_id,str) or not 1<=len(world_id)<=100:raise ValueError()
  return world_id
 except (ValueError,TypeError,KeyError,OSError,EOFError):raise ValueError('Invalid workspace timeline archive.') from None

def replace_workspace(service,archives):
 """Atomically replace the archived VH2 worlds in an existing local service.

 Normal character import deliberately refuses to overwrite a life. A global
 Horde domain restore is different: the browser host and the VH2 service must
 return to the same checkpoint, with the coordinator able to roll both back.
 This operation owns the complete VH2 world archive set. It replaces every
 world in the service, which makes an empty archive set a valid restore of an
 empty VH2 workspace and prevents post-backup worlds being left behind.
 """
 from contextlib import contextmanager
 decoded=workspace_archive_map(archives)
 with service.connect() as db:
  db.execute('BEGIN IMMEDIATE')
  class TransactionService:
   def __getattr__(self,name):return getattr(service,name)
   @contextmanager
   def connect(self):yield db
  # Events are append-only during normal play. A coordinator-approved restore
  # is the sole maintenance path allowed to replace an entire world history.
  db.execute('DROP TRIGGER IF EXISTS events_no_delete')
  try:
   world_ids=[row[0] for row in db.execute('SELECT id FROM worlds').fetchall()]
   for world_id in world_ids:
    db.execute('DELETE FROM vh2_provider_outputs WHERE job_id IN (SELECT id FROM vh2_provider_jobs WHERE world_id=?)',(world_id,))
    for table in ('photo_jobs','photo_assets','transcript_messages','media_records','memory_episodes','kernel_checkpoints','dialogue_jobs','vh2_provider_jobs','vh2_social_jobs','vh2_story_jobs','events'):
     db.execute('DELETE FROM '+table+' WHERE world_id=?',(world_id,))
    db.execute('DELETE FROM entity_views WHERE world_id=?',(world_id,))
    db.execute('DELETE FROM jobs WHERE world_id=?',(world_id,))
    db.execute("DELETE FROM commands WHERE json_extract(response,'$.worldId')=?",(world_id,))
   ...
   proxy=TransactionService()
   return {'worlds':[restore(proxy,data) for data in decoded.values()]}
  finally:
   # The surrounding connection rolls DDL and writes back on failure. Recreate
   # defensively for successful restores and for callers that recover from an
   # exception before SQLite leaves the transaction scope.
   db.execute('''CREATE TRIGGER IF NOT EXISTS events_no_delete BEFORE DELETE ON events
                 BEGIN SELECT RAISE(ABORT, 'World events are append-only'); END''')
