...
(function scenePulseWorldsModule(global) {
    ...
    function scenePulseViewPreferences(state) {
        return {
            ...
            theme: state.theme || 'default', fontScale: Number(state.fontScale) || 1, customPanels: clone(state.customPanels || []),
    ...
    function render(host, state) {
        ...
        host.dataset.scenepulseSource = state.mode === 'accepted_live' ? 'accepted_horde_reader_with_fixture_fallback' : 'TOUR_EXAMPLE_DATA'; host.dataset.scenePulseMode = state.mode; host.dataset.fixtureFallbackFields = Object.keys(state.origins || {}).filter(key => state.origins[key] === 'fixture').join(','); host.dataset.spTheme = state.theme;
        ...
        const effectRoot = effectsRoot(); effectRoot.dataset.spTheme = state.theme; effectRoot.style.setProperty('--sp-fs-base', `${Math.round(12 * state.fontScale)}px`); effectRoot.innerHTML = ambientEffects(state, mode);
        const root = overlayRoot(); root.dataset.spTheme = state.theme; root.style.setProperty('--sp-fs-base', `${Math.round(12 * state.fontScale)}px`); root.innerHTML = overlays;
    ...
    function bind(host, state, root) {
        ...
        const onChange = event => {
            ...
            if (target.dataset.action === 'theme') state.theme = target.value;
