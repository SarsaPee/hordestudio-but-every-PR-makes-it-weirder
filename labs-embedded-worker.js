...
self.onmessage = async event => {
    ...
    try {
        ...
        if (type === 'status') {
            ...
            const cached = registry?.is_pipeline_cached
                ? await registry.is_pipeline_cached('text-generation', model, { dtype: options.dtype || 'q4' })
                : loadedKey.startsWith(`${model}|`);
        ...
        if (type === 'remove') {
            ...
            const result = Transformers.ModelRegistry?.clear_pipeline_cache
